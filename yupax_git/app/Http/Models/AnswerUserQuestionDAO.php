...
class AnswerUserQuestionDAO extends BaseDAO {
    
    public function __construct() {
        parent::__construct("tb_answer_user_question");
    }
    
    public function question()
    {
        return $this->belongsTo(new AnswerQuestionDAO(),'questionId','id');
    }
    
    public function user()
    {
        return $this->belongsTo(new UserDAO(),'userId','id');
    }
    
    public function getList(AnswerQuestionForm $searchForm){
        try {
            $data = AnswerUserQuestionDAO::with(['question'])->select('*');
